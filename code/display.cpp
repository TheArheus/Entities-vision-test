...
bool InitWindow(void)
{
    ...
    LevelStripness = 10;
    GlobalFieldOfViewLOD = ColorBuffer->Height / LevelStripness;
...
std::vector<v2>
GenerateFieldOfView(i32 LOD, v2 EntityCenter, i32 Distance, r32 EntityRotation, r32 FieldOfViewAngle)
{
    std::vector<v2> Result;

    r32 RayAngle = EntityRotation - (FieldOfViewAngle / 2);
    Result.push_back(EntityCenter + Distance * V2(cosf(RayAngle), sinf(RayAngle)));
    for(u32 LODIndex = 0;
        LODIndex < LOD;
        ++LODIndex)
    {
        RayAngle += (FieldOfViewAngle / LOD);
        Result.push_back(EntityCenter + Distance * V2(cosf(RayAngle), sinf(RayAngle)));
    }
...
void 
DrawOutline(texture_t* RenderBuffer, v2 DrawStart, v2 Width, v2 Height, u32 LineWidth)
{
    // NOTE: Horizontal Lines
    DrawRect(RenderBuffer, DrawStart - LineWidth, DrawStart + Width + LineWidth, 0xFFFFFF00);
    DrawRect(RenderBuffer, DrawStart + Height - LineWidth, DrawStart + Width + Height + LineWidth, 0xFFFFFF00);

    // NOTE: Vertical Lines
    DrawRect(RenderBuffer, DrawStart - LineWidth, DrawStart + Height + LineWidth, 0xFFFFFF00);
    DrawRect(RenderBuffer, DrawStart + Width - LineWidth, DrawStart + Width + Height + LineWidth, 0xFFFFFF00);
}
...
void
DrawFieldOfView(r32 PlayerRotationAngle, v2 Start, r32 FieldOfView, i32 ViewDistance, u32 Color)
{
#if _PDEBUG
    r32 RayAngle1 = PlayerRotationAngle - (FieldOfView / 2);
    r32 RayAngle2 = RayAngle1 + (FieldOfView / 3);
    r32 RayAngle3 = RayAngle2 + (FieldOfView / 3);

    v2 LineMax1 = Start + ViewDistance * V2(cosf(RayAngle1), sinf(RayAngle1));
    v2 LineMax2 = Start + ViewDistance * V2(cosf(RayAngle2), sinf(RayAngle2));
    v2 LineMax3 = Start + ViewDistance * V2(cosf(RayAngle3), sinf(RayAngle3));

#if 0
    DrawRect(ColorBuffer, Start - 2, Start + 2, 0xFFFF0000);
    DrawRect(ColorBuffer, LineMax1 - 2, LineMax1 + 2, 0xFFFF0000);
    DrawRect(ColorBuffer, LineMax2 - 2, LineMax2 + 2, 0xFFFF0000);
    DrawRect(ColorBuffer, LineMax3 - 2, LineMax3 + 2, 0xFFFF0000);
#else
    DrawLine(ColorBuffer, Start, LineMax1, Color);
    DrawLine(ColorBuffer, Start, LineMax3, Color);
    DrawLine(ColorBuffer, LineMax1, LineMax2, Color);
    DrawLine(ColorBuffer, LineMax2, LineMax3, Color);
#endif

#else
    std::vector<v2> FieldOfViewVertices = GenerateFieldOfView(GlobalFieldOfViewLOD, Start, ViewDistance, PlayerRotationAngle, FieldOfView);
    DrawLine(ColorBuffer, Start, FieldOfViewVertices[0], Color);
    for(u32 VertIndex = 0;
        VertIndex < FieldOfViewVertices.size() - 1;
        ++VertIndex)
    {
        u32 CurrIndex = VertIndex;
        u32 NextIndex = VertIndex + 1;
        DrawLine(ColorBuffer, FieldOfViewVertices[CurrIndex], FieldOfViewVertices[NextIndex], Color);
    }
    DrawLine(ColorBuffer, FieldOfViewVertices[FieldOfViewVertices.size() - 1], Start, Color);
...
void
DrawPointsOfPolygon(std::vector<v2> Vertices, u32 Color)
{
    for(u32 PIndex = 0;
        PIndex < Vertices.size();
        ++PIndex)
    {
        v2 P = Vertices[PIndex];
        DrawRect(ColorBuffer, P - 2, P + 2, Color);
    }
}

void
DrawPolygon(std::vector<v2> Vertices, u32 Color)
{
    for(u32 PIndex = 0;
        PIndex < Vertices.size();
        ++PIndex)
